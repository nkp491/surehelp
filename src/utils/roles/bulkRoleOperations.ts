  ...
  try {
    ...
    if (!isAdmin) {
      return {
        success: false,
        message:
          "You must have system_admin role to perform bulk role operations",
      };
    }
    ...
    for (const userId of userIds) {
      ...
      if (action === "assign") {
        ...
        const data = await deleteStripeSubscription(
          user?.stripe_subscription_id
        );
        if (data?.error) {
          return {
            success: false,
            message: "Failed to delete Stripe subscription for user",
          };
        }
    ...
    if (failCount === 0) {
      ...
      } role ${role} for all ${successCount} users`;
