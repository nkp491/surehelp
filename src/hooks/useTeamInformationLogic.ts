  ...
  const { 
    ...
    queryFn: async () => {
      try {
        ...
        // Method 1: Direct fetch - this should be the primary method
        ...
        // Method 2: Fetch through manager - fallback if direct fetch fails
        if (managerId) {
        ...
        // Method 3: Try with no RLS - last resort
    ...
    const checkTeamVisibility = async () => {
      try {
        ...
        if (managerId && userTeams.length === 0 && !isLoadingTeams && !fixingTeamAssociation && !isProcessing) {
          ...
          // Standard team association - try to force an association
