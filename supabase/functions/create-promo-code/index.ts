...
serve(async (req)=>{
  ...
  try {
    ...
    if (activateCode) {
      ...
      console.log(`Action: ${action}, Promo ID: ${promo_id}`);
    ...
    if (body.deleteCode) {
      ...
      const { data, error } = await supabaseClient.from("promo_codes").delete().eq("coupon_id", coupon_id).select("*");
      if (error) {
